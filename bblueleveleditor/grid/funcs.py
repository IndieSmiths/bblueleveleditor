...
def generate_grid_lines(
    ...
    area_rect
        pygame.Rect instance representing the area covered
        by the grid.
    offset (2-tuple of integers)
        offset applied to lines.
    use_vectors
    ...
    ### check whether unit dimensions are smaller
    ### than the full area dimensions
    ...
    # TODO review why we must subtract one from bottom
    # and right and explain it in a comment at this
    # spot; also update other forks using our3rdlibs.grid;

    top, bottom = area_rect.top, area_rect.bottom - 1
    left, right = area_rect.left, area_rect.right - 1
    ...
    ### store start and end point pairs for each
    ### orientation, adding the offset

    dx, dy = offset

    vert_start_points = [(x + dx, top + dy) for x in x_sections]

    vert_end_points = [(x + dx, bottom + dy) for x in x_sections]

    horiz_start_points = [(left + dx, y + dy) for y in y_sections]

    horiz_end_points = [(right + dx, y + dy) for y in y_sections]
    ...
    ### if requested, discard the first line in each
    ### orientation
